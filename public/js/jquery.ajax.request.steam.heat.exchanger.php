<?php # НАЗНАЧЕНИЕ: ОБРАБОТЧИК ОНЛАЙН-СЕРВИСА "ЗАЯВКА НА РАСЧЕТ И ПОДБОР ПАРОВОГО ТЕПЛООБМЕННИКА"

# https://euroheater.ru/raschet-parovogo-teploobmennika/
...

{
    ...
    # 'Заявка на расчет и подбор парового теплообменника:'.PHP_EOL.
    ...
    (!empty($_POST['steamTemperature']) ? 'Температура пара, °C: '.$_POST['steamTemperature'].PHP_EOL : '').
	...
	'Zayavka na raschet i podbor parovogo teploobmennika: '.PHP_EOL.
    ...
    (!empty($_POST['sizesLaneEn']) ? 'Dannie po razmeram. Ryadnost: '.$_POST['sizesLaneEn'].PHP_EOL : '').
    ...
    (!empty($_POST['steamTemperatureEn']) ? 'Temperature para, °C: '.$_POST['steamTemperatureEn'].PHP_EOL : '').
    ...
    */
    ...
    str_replace('www.', '', $_SERVER['SERVER_NAME']).' - заявка на расчет и подбор парового теплообменника ['.date("j.n.Y G:i").']',
