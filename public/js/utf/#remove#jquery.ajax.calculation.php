<?php # НАЗНАЧЕНИЕ: ОБРАБОТЧИК ФОРМЫ "РАСЧЕТ КАЛЬКУЛЯЦИИ НА АРЕНДУ ДЭС":
# https://euroheater.ru/raschet/
# дата создания: 2016.07.14
...
function sendLetterToManager()
{
	...
	$message =
    "Заявка на расчет стоимости аренды:".PHP_EOL.
	"Имя: ".$_POST['name'].PHP_EOL.
	"Телефон для связи: ".$_POST['phone'].PHP_EOL.
	"Мощность: ".$_POST['power'].PHP_EOL.
	"Длительность: ".$_POST['duration'].PHP_EOL.
	"Заявка отправлена со страницы: ".$_POST['url'].PHP_EOL
	;
    
	$messageEn =
	PHP_EOL."***** Tekst-v-transliteracii *****:".PHP_EOL.
	"Zayavka na raschet stoimosti arendy:".PHP_EOL.
    "Imya: ".$_POST['name_en'].PHP_EOL.
	"Telefon dlya svyazi: ".$_POST['phone_en'].PHP_EOL.
    "Moshnost: ".$_POST['power_en'].PHP_EOL.
    "Dlitelnost: ".$_POST['duration_en'].PHP_EOL.
    ...
    DOMAIN_SHORT.' - заявка на расчет стоимости аренды ['.date('j '.getRusMonthName(date('m')).' Y G:i').']',
	$message.$messageEn,
	...
	else
	{
		...
		К сожалению, сервис расчета аренды ДЭС через сайт временно не работает.
		<br /><br />
