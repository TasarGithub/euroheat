<?php # НАЗНАЧЕНИЕ: ОБРАБОТЧИК ОНЛАЙН-СЕРВИСА "РАССЧИТАТЬ СТОИМОСТЬ" НА ГЛАВНОЙ И ВНУТРЕННИХ
# дата создания: 2018.01.10
...
if (empty($_POST['name']) || empty($_POST['email']) || empty($_POST['phone'])) exit('');
...
function sendLetterToManager()
{
	...
	$message =
    # "Заявка на расчет стоимости:".PHP_EOL.
	"Имя: ".$_POST['name'].PHP_EOL.
	"Телефон для связи: ".$_POST['phone'].PHP_EOL.
	"Телефон для связи: ".$_POST['email'].PHP_EOL.
    ...
    DOMAIN_SHORT.' - заявка на расчет стоимости ['.date('j '.getRusMonthName(date('m')).' Y G:i').']',
	$message.$messageEn,
	...
	else
	{
		...
		К сожалению, сервис расчета стоимости через сайт временно не работает.
		<br /><br />
