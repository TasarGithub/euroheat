<?php # НАЗНАЧЕНИЕ: ОБРАБОТЧИК ОНЛАЙН-СЕРВИСА "ЗАЯВКА НА РАСЧЕТ ВОДЯНОГО НАГРЕВАТЕЛЯ"
# https://euroheater.ru/raschet-nagrevatelya/
...
function sendLetterToManager()
{
	...
	$message =
    # 'Заявка на расчет водяного нагревателя:'.PHP_EOL.
    ...
    (!empty($_POST['coolantType']) ? 'Тип теплоносителя: '.$_POST['coolantType'].PHP_EOL : '').
	...
	'Zayavka na raschet vodyanogo nagrevatela: '.PHP_EOL.
    ...
    (!empty($_POST['tubeMaterialEn']) ? 'Materiali. Material trubki: '.$_POST['tubeMaterialEn'].PHP_EOL : '').
    ...
    (!empty($_POST['coolantTypeEn']) ? 'Tehnicheskoe zadanie. Tip teplonositela: '.$_POST['coolantTypeEn'].PHP_EOL : '').
    (!empty($_POST['inputCoolantTemperatureEn']) ? 'Tehnicheskoe zadanie. Temperatura teplonositela na vhode: '.$_POST['inputCoolantTemperatureEn'].PHP_EOL : '').
    (!empty($_POST['outputCoolantTemperatureEn']) ? 'Tehnicheskoe zadanie. Temperatura teplonositela na vihode: '.$_POST['outputCoolantTemperatureEn'].PHP_EOL : '').
    ...
    str_replace('www.', '', $_SERVER['SERVER_NAME']).' - заявка на расчет водяного нагревателя ['.date("j.n.Y G:i").']',
	# $message.$messageEn,
	$message,
