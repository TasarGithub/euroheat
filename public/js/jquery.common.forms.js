...
$(document).ready(function () {
  ...
  $("#callback_modal").on("shown.bs.modal", function () {
  ...
  $("#rent_modal").on("shown.bs.modal", function () {
  ...
  // Устанавливаем фокус в модальном окне "Добавить отзыв"
  $("#full_price_modal").on("shown.bs.modal", function () {
  ...
  $("#callback_form_name, #callback_form_phone").keypress(function (e) {
    if (e.which == 13) {
      $("#callback_form_submit_button").click();
      return false;
    }
  });
  ...
  $("#callback_form_submit_button").click(function () {
    ...
    if (!phone || (phone.match(/\d/g) ? phone.match(/\d/g).length : 0) < 10) {
      showMessage(
        "#callback_form_phone",
        "Пожалуйста, укажите свой телефон с кодом.<br />Например: (495) 134-23-53."
      );
    ...
    if (name && phone) {
      $.post(
        "/public/js/jquery.ajax.callback.php",
        {
          name: name,
  ...
  $("#calc_form_submit_button").click(function () {
    ...
    if (name && phone) {
      $.post(
        "/public/js/jquery.ajax.calculate.cost-of-rent.php",
        {
          name: name,
          name_en: convertRuLettersToEn(name),
          phone: phone,
  ...
  $("#calculation_form_name, #calculation_form_phone").keypress(function (e) {
    if (e.which == 13) {
      $("#calculation_form_submit_button").click();
      return false;
    }
  });
  ...
  $("#calculation_form_submit_button").click(function () {
    ...
    if (!name || name.length < 3 || !hasDifferentLetters(name)) {
      showMessage(
        "#calculation_form_name",
        "Пожалуйста, укажите контактное имя."
      );
    ...
    var duration = $.trim(
      $("#calculation_form_duration option:selected").text()
    );

    if (name && phone) {
      $.post(
        "/public/js/jquery.ajax.calculation.php",
        {
          name: name,
          name_en: convertRuLettersToEn(name),
          phone: phone,
          phone_en: convertRuLettersToEn(phone),
          power: power,
          power_en: convertRuLettersToEn(power),
          duration: duration,
          duration_en: convertRuLettersToEn(duration),
          url: window.location.href,
        },
        function (data) {
          if (data) {
  ...
  // модальное окно Отправить заявку главная страница. Обработчик нажатия клавиши Enter
  $("#rent_form_name, #rent_form_phone").keypress(function (e) {
    if (e.which == 13) {
      $("#rent_form_submit_button").click();
      return false;
    }
  });
  ...
  $("#rent_form_submit_button").click(function () {
    ...
    // E-mail
    var email = $.trim($("#rent_form_email").val());
    if (!email || !/@/.test(email) || !isEmailValid(email)) {
      showMessage("#rent_form_email", "Пожалуйста, укажите свой E-mail");
      $("#rent_form_email").focus();
      return false;
    }

    // if (name && email && phone) {
    //   $.post(
    //     "/public/js/jquery.ajax.full.price.php",
    //     {
    //       name: name,
    //       name_en: convertRuLettersToEn(name),
    //       email: email,
    //       email_en: convertRuLettersToEn(email),
    //       phone: phone,
    //       phone_en: convertRuLettersToEn(phone),
    //       url: window.location.href,
    //     },
    ...
    if (name && email && phone) {
      $.post(
        "/public/js/jquery.ajax.order.rent.php",
        {
          name: name,
          name_en: convertRuLettersToEn(name),
          email: email,
  ...
  $("#feedback_form_name, #feedback_form_activity").keypress(function (e) {
    if (e.which == 13) {
      $("#feedback_form_submit_button").click();
      return false;
    }
  });
  ...
  $("#feedback_form_submit_button").click(function () {
    ...
    if (!activity) {
      showMessage(
        "#feedback_form_activity",
        "Пожалуйста, укажите сферу деятельности."
      );
    ...
    if (name && activity && text) {
      $.post(
        "/public/js/jquery.ajax.feedback.add.php",
        {
          name: name,
          name_en: convertRuLettersToEn(name),
          activity: activity,
          activity_en: convertRuLettersToEn(activity),
          text: text,
          text_en: convertRuLettersToEn(text),
          url: window.location.href,
        },
        function (data) {
          if (data) {
  ...
  $("#contacts_form_name, #contacts_form_phone").keypress(function (e) {
    if (e.which == 13) {
      $("#contacts_form_submit_button").click();
      return false;
    }
  });
  ...
  $("#contacts_form_submit_button").click(function () {
    ...
    if (name && phone && email && text) {
      $.post(
        "/public/js/jquery.ajax.contacts.php",
        {
          name: name,
          name_en: convertRuLettersToEn(name),
          phone: phone,
  ...
  $(
    "#full_price_modal_name, #full_price_modal_email, #full_price_modal_phone"
  ).keypress(function (e) {
    if (e.which == 13) {
      $("#full_price_modal_submit_button").click();
      return false;
    }
  });
  ...
  $("#full_price_modal_submit_button").click(function () {
    ...
    if (!email || !/@/.test(email) || !isEmailValid(email)) {
      // Прокручиваем
      $("html, body").animate(
        { scrollTop: $("#full_price_modal_email").offset().top - 150 },
        100
      );
  ...
  $(
    "#request_form_name, #request_form_company, #request_form_city, #request_form_email, #request_form_phone"
  ).keypress(function (e) {
    if (e.which == 13) {
      $("#request_form_submit_button").click();
      return false;
    }
  });
  ...
  $("#request_form_submit_button").on("click", function () {
    ...
    // Тип теплообменника
    var heatExchangerType = $(
      "input[name=request_form_heat_exchanger_type]:checked"
    ).val();
    ...
    var inputAirTemperature = $.trim(
      $("#request_form_input_air_temperature").val()
    );

    // Температура воздуха на выходе
    var outputAirTemperature = $.trim(
      $("#request_form_output_air_temperature").val()
    );

    // Температура теплоносителя на входе
    var inputCoolantTemperature = $.trim(
      $("#request_form_input_coolant_temperature").val()
    );

    // Температура теплоносителя на выходе
    var outputCoolantTemperature = $.trim(
      $("#request_form_output_coolant_temperature").val()
    );
    ...
    if (!name || name.length < 3 || !hasDifferentLetters(name)) {
      // Прокручиваем
      $("html, body").animate(
        { scrollTop: $("#request_form_name").offset().top - 150 },
        100
      );
    ...
    if (!email || !/@/.test(email) || !isEmailValid(email)) {
      // Прокручиваем
      $("html, body").animate(
        { scrollTop: $("#request_form_email").offset().top - 150 },
        100
      );
    ...
    if (!phone || (phone.match(/\d/g) ? phone.match(/\d/g).length : 0) < 10) {
      // Прокручиваем
      $("html, body").animate(
        { scrollTop: $("#request_form_phone").offset().top - 150 },
        100
      );
      showMessage(
        "#request_form_phone",
    ...
    if (name && email && phone) {
      $.post(
        "/public/js/jquery.ajax.request.heat.exchanger.php",
        {
          ...
          inputCoolantTemperatureEn: convertRuLettersToEn(
            inputCoolantTemperature
          ),
          outputCoolantTemperature: outputCoolantTemperature,
          outputCoolantTemperatureEn: convertRuLettersToEn(
            outputCoolantTemperature
          ),
          power: power,
  ...
  $(
    "#water_heater_form_name, #water_heater_form_company, #water_heater_form_city, #water_heater_form_email, #water_heater_form_phone"
  ).keypress(function (e) {
    if (e.which == 13) {
      $("#water_heater_form_submit_button").click();
      return false;
    }
  });
  ...
  $("#water_heater_form_submit_button").on("click", function () {
    ...
    var inputAirTemperature = $(
      "#water_heater_form_input_air_temperature"
    ).val();
    // Техническое задание: температура воздуха на выходе
    var outputAirTemperature = $(
      "#water_heater_form_output_air_temperature"
    ).val();
    ...
    var inputCoolantTemperature = $(
      "#water_heater_form_input_coolant_temperature"
    ).val();
    // Техническое задание: температура теплоносителя на выходе
    var outputCoolantTemperature = $(
      "#water_heater_form_output_coolant_temperature"
    ).val();
    ...
    if (!name || name.length < 3 || !hasDifferentLetters(name)) {
      // Прокручиваем
      $("html, body").animate(
        { scrollTop: $("#water_heater_form_name").offset().top - 150 },
        100
      );
      showMessage(
        "#water_heater_form_name",
        "Пожалуйста, укажите контактное имя."
      );
    ...
    if (!email || !/@/.test(email) || !isEmailValid(email)) {
      // Прокручиваем
      $("html, body").animate(
        { scrollTop: $("#water_heater_form_email").offset().top - 150 },
        100
      );
      showMessage(
        "#water_heater_form_email",
    ...
    if (!phone || (phone.match(/\d/g) ? phone.match(/\d/g).length : 0) < 10) {
      // Прокручиваем
      $("html, body").animate(
        { scrollTop: $("#water_heater_form_phone").offset().top - 150 },
        100
      );
      showMessage(
        "#water_heater_form_phone",
        "Пожалуйста, укажите свой телефон с кодом.<br />Например: (495) 134-23-53."
      );
    ...
    if (name && email && phone) {
      $.post(
        "/public/js/jquery.ajax.request.water.heater.php",
        {
          sizesFTA: sizesFTA,
  ...
  $(
    "#water_cooler_form_name, #water_cooler_form_company, #water_cooler_form_city, #water_cooler_form_email, #water_cooler_form_phone"
  ).keypress(function (e) {
    if (e.which == 13) {
      $("#water_cooler_form_submit_button").click();
      return false;
    }
  });
  ...
  $("#water_cooler_form_submit_button").on("click", function () {
    ...
    // Техническое задание: температура воздуха на входе
    var inputAirTemperature = $(
      "#water_cooler_form_input_air_temperature"
    ).val();
    // Техническое задание: температура воздуха на выходе
    var outputAirTemperature = $(
      "#water_cooler_form_output_air_temperature"
    ).val();
    // Техническое задание: температура теплоносителя на входе
    var inputCoolantTemperature = $(
      "#water_cooler_form_input_coolant_temperature"
    ).val();
    // Техническое задание: температура теплоносителя на выходе
    var outputCoolantTemperature = $(
      "#water_cooler_form_output_coolant_temperature"
    ).val();
    // Техническое задание: мощность
    ...
    if (!name || name.length < 3 || !hasDifferentLetters(name)) {
      // Прокручиваем
      $("html, body").animate(
        { scrollTop: $("#water_cooler_form_name").offset().top - 150 },
        100
      );
      showMessage(
        "#water_cooler_form_name",
        "Пожалуйста, укажите контактное имя."
      );
    ...
    if (!email || !/@/.test(email) || !isEmailValid(email)) {
      // Прокручиваем
      $("html, body").animate(
        { scrollTop: $("#water_cooler_form_email").offset().top - 150 },
        100
      );
      showMessage(
        "#water_cooler_form_email",
        "Пожалуйста, укажите свой E-mail"
      );
    ...
    if (!phone || (phone.match(/\d/g) ? phone.match(/\d/g).length : 0) < 10) {
      // Прокручиваем
      $("html, body").animate(
        { scrollTop: $("#water_cooler_form_phone").offset().top - 150 },
        100
      );
      showMessage(
        "#water_cooler_form_phone",
        "Пожалуйста, укажите свой телефон с кодом.<br />Например: (495) 134-23-53."
      );
    ...
    if (name && email && phone) {
      $.post(
        "/public/js/jquery.ajax.request.water.cooler.php",
        {
          sizesFTA: sizesFTA,
          sizesFTAEn: convertRuLettersToEn(sizesFTA),
          sizesFTA: sizesFTA,
  ...
  $(
    "#evaporator_form_name, #evaporator_form_company, #evaporator_form_city, #evaporator_form_email, #evaporator_form_phone"
  ).keypress(function (e) {
    if (e.which == 13) {
      $("#evaporator_form_submit_button").click();
      return false;
    }
  });
  ...
  $("#evaporator_form_submit_button").on("click", function () {
    ...
    // Техническое задание: температура воздуха на выходе
    var outputAirTemperature = $(
      "#evaporator_form_output_air_temperature"
    ).val();
    ...
    var freonEvaporationTemperature = $(
      "#evaporator_form_freon_evaporation_temperature"
    ).val();
    // Техническое задание: мощность
    ...
    if (!name || name.length < 3 || !hasDifferentLetters(name)) {
      // Прокручиваем
      $("html, body").animate(
        { scrollTop: $("#evaporator_form_name").offset().top - 150 },
        100
      );
      showMessage(
        "#evaporator_form_name",
        "Пожалуйста, укажите контактное имя."
      );
    ...
    if (!email || !/@/.test(email) || !isEmailValid(email)) {
      // Прокручиваем
      $("html, body").animate(
        { scrollTop: $("#evaporator_form_email").offset().top - 150 },
        100
      );
    ...
    if (!phone || (phone.match(/\d/g) ? phone.match(/\d/g).length : 0) < 10) {
      // Прокручиваем
      $("html, body").animate(
        { scrollTop: $("#evaporator_form_phone").offset().top - 150 },
        100
      );
      showMessage(
        "#evaporator_form_phone",
        "Пожалуйста, укажите свой телефон с кодом.<br />Например: (495) 134-23-53."
      );
    ...
    if (name && email && phone) {
      $.post(
        "/public/js/jquery.ajax.request.evaporator.php",
        {
          sizesFTA: sizesFTA,
          sizesFTAEn: convertRuLettersToEn(sizesFTA),
          sizesFTA: sizesFTA,
          ...
          freonEvaporationTemperatureEn: convertRuLettersToEn(
            freonEvaporationTemperature
          ),
          power: power,
          powerEn: convertRuLettersToEn(power),
          notes: notes,
  ...
  $(
    "#refrigerator_form_name, #refrigerator_form_company, #refrigerator_form_city, #refrigerator_form_email, #refrigerator_form_phone"
  ).keypress(function (e) {
    if (e.which == 13) {
      $("#refrigerator_form_submit_button").click();
      return false;
    }
  });
  ...
  $("#refrigerator_form_submit_button").on("click", function () {
    ...
    // Техническое задание: температура воздуха на входе
    var inputAirTemperature = $(
      "#refrigerator_form_input_air_temperature"
    ).val();
    // Техническое задание: температура воздуха на выходе
    var outputAirTemperature = $(
      "#refrigerator_form_output_air_temperature"
    ).val();
    // Техническое задание: тип фреона
    var freonType = $("#refrigerator_form_freon_type").val();
    // Техническое задание: температура испарения фреона, °C
    var freonEvaporationTemperature = $(
      "#refrigerator_form_freon_evaporation_temperature"
    ).val();
    // Техническое задание: мощность
    ...
    if (!name || name.length < 3 || !hasDifferentLetters(name)) {
      // Прокручиваем
      $("html, body").animate(
        { scrollTop: $("#refrigerator_form_name").offset().top - 150 },
        100
      );
      showMessage(
        "#refrigerator_form_name",
        "Пожалуйста, укажите контактное имя."
      );
    ...
    if (!email || !/@/.test(email) || !isEmailValid(email)) {
      // Прокручиваем
      $("html, body").animate(
        { scrollTop: $("#refrigerator_form_email").offset().top - 150 },
        100
      );
      showMessage(
        "#refrigerator_form_email",
        "Пожалуйста, укажите свой E-mail"
      );
    ...
    if (!phone || (phone.match(/\d/g) ? phone.match(/\d/g).length : 0) < 10) {
      // Прокручиваем
      $("html, body").animate(
        { scrollTop: $("#refrigerator_form_phone").offset().top - 150 },
        100
      );
      showMessage(
        "#refrigerator_form_phone",
        "Пожалуйста, укажите свой телефон с кодом.<br />Например: (495) 134-23-53."
      );
    ...
    if (name && email && phone) {
      $.post(
        "/public/js/jquery.ajax.request.refrigerator.php",
        {
          sizesFTA: sizesFTA,
          sizesFTAEn: convertRuLettersToEn(sizesFTA),
          sizesFTA: sizesFTA,
  ...
  // Форма "Расчет водяного нагревателя". Обработчик нажатия клавиши Enter
  $(
    "#steam_heat_exchanger_form_name, #steam_heat_exchanger_form_company, #steam_heat_exchanger_form_city, #steam_heat_exchanger_form_email, #steam_heat_exchanger_form_phone"
  ).keypress(function (e) {
    if (e.which == 13) {
      $("#steam_heat_exchanger_form_submit_button").click();
      return false;
    }
  });
  ...
  $("#steam_heat_exchanger_form_submit_button").on("click", function () {
    ...
    // Материалы: материал ламелей
    var lamellaMaterial = $(
      "#steam_heat_exchanger_form_lamella_material"
    ).val();
    ...
    // Техническое задание: температура воздуха на входе
    var inputAirTemperature = $(
      "#steam_heat_exchanger_form_input_air_temperature"
    ).val();
    // Техническое задание: температура воздуха на выходе
    var outputAirTemperature = $(
      "#steam_heat_exchanger_form_output_air_temperature"
    ).val();
    // Техническое задание: температура пара
    var steamTemperature = $(
      "#steam_heat_exchanger_form_steam_temperature"
    ).val();
    // Техническое задание: мощность
    ...
    if (!name || name.length < 3 || !hasDifferentLetters(name)) {
      // Прокручиваем
      $("html, body").animate(
        { scrollTop: $("#steam_heat_exchanger_form_name").offset().top - 150 },
        100
      );
      showMessage(
        "#steam_heat_exchanger_form_name",
        "Пожалуйста, укажите контактное имя."
      );
    ...
    if (!email || !/@/.test(email) || !isEmailValid(email)) {
      // Прокручиваем
      $("html, body").animate(
        { scrollTop: $("#steam_heat_exchanger_form_email").offset().top - 150 },
        100
      );
      showMessage(
        "#steam_heat_exchanger_form_email",
        "Пожалуйста, укажите свой E-mail"
      );
    ...
    if (!phone || (phone.match(/\d/g) ? phone.match(/\d/g).length : 0) < 10) {
      // Прокручиваем
      $("html, body").animate(
        { scrollTop: $("#steam_heat_exchanger_form_phone").offset().top - 150 },
        100
      );
      showMessage(
        "#steam_heat_exchanger_form_phone",
        "Пожалуйста, укажите свой телефон с кодом.<br />Например: (495) 134-23-53."
      );
    ...
    if (name && email && phone) {
      $.post(
        "/public/js/jquery.ajax.request.steam.heat.exchanger.php",
        {
          sizesFTA: sizesFTA,
          sizesFTAEn: convertRuLettersToEn(sizesFTA),
          sizesFTA: sizesFTA,
