$(document).ready(function(){ // jquery ready
	// дл€ формы добавлени€ раздела сайта
    if (isGetVarExists('action') == 'addItem') {
		$('#site_sections_form_name').focus();
	}
	// дл€ формы редактировани€ раздела сайта
    ...
    // субмит формы добавлени€ раздела сайта
    ...
    $('#site_sections_form_name').on('change keyup click', function(event) { checkExistenceByName(event); }); // /изменени€ в поле "Ќазвание раздела сайта"
    
    // изменени€ в select'е "ѕринадлежность"
                ...
                if (data) {
                    try {
                        ...
                        if (result['result'] == 'exists') {
                            $('#site_sections_form_name_alert_div').html('¬ базе уже существует раздел с названием "' + name + '" и с принадлежностью к "'+ $.trim($('#site_sections_form_parent_id option:selected').text().replace(/-/g, '').replace('ї', '')) +'": <a href="/control/site_sections/?action=editItem&itemID=' + result['id'] + '" target="_blank">смотреть</a>.<br />ѕжл, укажите другое название дл€ раздела сайта.').removeClass('hidden');
                            $('#site_sections_form_name').focus();
                        }
            ...
            $.post('/control/site_sections/ajax.php', { 'action': 'check_item_for_existence_by_url', 'url': url, 'id': id, 'parent_id': parent_id }, function(data) {
                if (data) {
                    try {
                        var result = JSON.parse(data); // console.log('%o', result);
                        if (result['result'] == 'exists') {
                            $('#site_sections_form_url_alert_div').html('¬ базе уже существует раздел с директорией "' + url + '" и с принадлежностью к "'+ $.trim($('#site_sections_form_parent_id option:selected').text().replace(/-/g, '').replace('ї', '')) +'": <a href="/control/site_sections/?action=editItem&itemID=' + result['id'] + '" target="_blank">смотреть</a>.<br />ѕжл, укажите другую директорию дл€ раздела.').removeClass('hidden');
    ...
    // /изменени€ в поле "»м€ файла"
	
    // Ўаблоны: ссылка "—охранить изменени€"
    $(document.body).delegate('.site_sections_edit_save_changes', 'click', function() {
        // провер€ем пол€ формы на заполненность
        if (checkForm('#site_sections_edit_form')) {
            // сохран€ем информацию через ajax
            $.post('/control/site_sections/ajax.php', { 'action': 'edit_item_submit', 'params': decodeURI($('#site_sections_edit_form').serialize()) }, function(data) {
                ...
                if (data == 'success') var dataResult = '<div class="alert alert-success alert_line">изменени€ успешно сохранены (' + lastUpdateDate + ')</div>';
                else var dataResult = '<div class="alert alert-danger alert_line">изменени€ Ќ≈ сохранены, произошла ошибка (' + lastUpdateDate + ')</div>';
    ...
    // при ошибке добавлени€ раздела, выделаем нужный option в select'е
