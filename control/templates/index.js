$(document).ready(function(){
    ...
    if (isGetVarExists('action') == 'addItem') {
        $('#templates_form_name').on('change keyup click', function() {
            ...
            if (name.length && name != old_value) {
                ...
                $.post('/control/templates/ajax.php', { 'action': 'check_template_for_existence_by_name', 'name': name }, function(data) {
                    if (data) {
                        try {
                            var result = JSON.parse(data); // console.log('%o', result);
                            if (result['result'] == 'exists') {
                                $('#templates_form_name_alert_div').html('В базе уже существует шаблон с указанным названием: <a href="/control/templates/?action=editItem&itemID=' + result['id'] + '" target="_blank">смотреть</a>.<br />Пжл, укажите другое название для шаблона.').removeClass('hidden');
                                $('#templates_form_name').focus();
                            }
    ...
    $('#templates_form_file_name').on('change keyup click', function() {
        ...
        if (file_name.length && file_name != old_value) {
            ...
            $.ajaxQ.abortAll();
            // провряем, есть ли шаблон в базе с указанным именем
            $.post('/control/templates/ajax.php', { 'action': 'check_template_for_existence_by_file_name', 'file_name': file_name }, function(data) {
                if (data) {
                    try {
                        var result = JSON.parse(data); // console.log('%o', result);
                        if (result['result'] == 'exists') {
                            $('#templates_form_file_name_alert_div').html('В базе уже существует шаблон с указанным именем файла: <a href="/control/templates/?action=editItem&itemID=' + result['id'] + '" target="_blank">смотреть</a>.<br />Пжл, укажите другое имя файла.').removeClass('hidden');
    ...
    function checkFileExistence() {
        ...
        $.post('/control/templates/ajax.php', { 'action': 'checkTemplateForErrorsAddItem', 'file_name': $.trim($('#templates_form_file_name').val()) }, function(data) {
            try {
                var result = JSON.parse(data); // console.log('%o', result);
                if (result['result'] == 'exists') {
    ...
    $(document.body).delegate('#templates_edit_save_changes', 'click', function(){
        ...
        if (checkForm('#templates_edit_form')) {
            ...
            $.post('/control/templates/ajax.php', { 'action': 'edit_template_submit', 'name': $('#templates_form_name').val(), 'html_code': $('#templates_form_html_code').val(), 'url': $('#templates_form_file_url').val(), 'id': $('#template_id').val() }, function(data) {
                var d = new Date();
	...
	$('#searchByName').on('keyup', function(){
		$('#resultSet').load('/control/templates/ajax.php', '\
	...
	$('.checkForErrors').click(function()
	{
		...
		if (isError)
		{
			alert('Необходимо устранить ошибку. После чего можно будет сохранить шаблон.');
			return false;
		}
	})
	...
	function getAllBackups(){
		...
		$.ajax({
				url: "/control/templates/ajax.php", // обращение
				type: 'POST',
