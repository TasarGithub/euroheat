$(document).ready(function () {
              ...
              try {
                ...
                if (result["result"] == "exists") {
                  $("#templates_form_name_alert_div")
                    .html(
                      'В базе уже существует шаблон с указанным названием: <a href="/control/templates/?action=editItem&itemID=' +
                        result["id"] +
                        '" target="_blank">смотреть</a>.<br />Пжл, укажите другое название для шаблона.'
                    )
                    .removeClass("hidden");
            ...
            try {
              ...
              if (result["result"] == "exists") {
                $("#templates_form_file_name_alert_div")
                  .html(
                    'В базе уже существует шаблон с указанным именем файла: <a href="/control/templates/?action=editItem&itemID=' +
                      result["id"] +
                      '" target="_blank">смотреть</a>.<br />Пжл, укажите другое имя файла.'
                  )
                  .removeClass("hidden");
      ...
      if (checkForm("#templates_edit_form")) {
        ...
        $.post(
          ...
          function (data) {
            ...
            if (data == "success")
              var dataResult =
                '<div class="alert alert-success alert_line">изменения успешно сохранены (' +
                lastUpdateDate +
                ")</div>";
            else
              var dataResult =
                '<div class="alert alert-danger alert_line">изменения НЕ сохранены, произошла ошибка (' +
                lastUpdateDate +
                ")</div>";
  ...
  $(".checkForErrors").click(function () {
    ...
    if (isError) {
      alert(
        "Необходимо устранить ошибку. После чего можно будет сохранить шаблон."
      );
  ...
  function getAllBackups() {
    ...
    $.ajax({
      ...
      error: function (XMLHttpRequest, textStatus, errorThrown) {
        alert(
          "Ошибка при выполнении ajax-запроса. Пожалуйста, обратитесь к администратору сайта."
        );
      },
    });
