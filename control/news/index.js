            ...
            $.post('/control/news/ajax.php', { 'action': 'check_item_for_existence_by_date_add', 'id': isGetVarExists('itemID'), 'date_add': date_add }, function(data) {
                if (data) {
                    try {
                        var result = JSON.parse(data); // console.log('%o', result);
                        if (result['result'] == 'exists') {
                            $('#news_form_date_add_alert_div').html('В базе уже существует новость с датой "' + date_add + '": <a href="/control/news/?action=editItem&itemID=' + result['id'] + '" target="_blank">смотреть</a>.<br />Пжл, укажите другую дату для новости.').removeClass('hidden');
