...
# ЛОГИКА
# провряем, существует ли вопрос-ответ по названию
    ...
    if ($_ = $sth->fetchAll())
    {
        ...
        for ($i=0;$i<$_c;$i++)
        {
            ...
            <tr'.$trClass.'>
                ...
                <td>'.$_[$i]['name'].'</td>
                <td class="center vertical_middle">
                    <a class="block" title="Удалить вопрос-ответ" href="/control/faq/?action=deleteItem&itemID='.$_[$i]['id'].'" onClick="return confirm(\'Вопрос-ответ будет удалена безвозвратно. Удалить вопрос-ответ?\')">
