...
(function ($) {
  ...
  $.backup = function (inputs_list) {
    // console.log("inputs_list: ", inputs_list);
    // проверка переменных

    if (inputs_list === "undefined") {
      console.log(
        'jquery module "backup" can not start because "inputs_list" variable is not defined.'
      );
    ...
    if (inputs_list.table_name === "undefined") {
      console.log(
        'jquery module "backup" can not start because "table_name" variable is not defined.'
      );
    ...
    if (inputs_list.entry_id === "undefined") {
      console.log(
        'jquery module "backup" can not start because "entry_id" variable is not defined.'
      );
    ...
    if (inputs_list.fields_name === "undefined") {
      console.log(
        'jquery module "backup" can not start because "fields_name" variable is not defined.'
      );
    ...
    $.post(
      "/control/public/js/jquery.backup/jquery.backup.ajax.php",

      { action: "getAllBackups", inputs_list: inputs_list },
      ...
      function (data) {
        ...
        if (data) var result = JSON.parse(data);
        ...
        $.each(inputs_list.fields_name, function (index, value) {
          ...
          var content =
            '<div class="jquery_backups" data-field-id="' +
            value +
            '" data-table-name="' +
            inputs_list.table_name +
            '" data-entry-id="' +
            inputs_list.entry_id +
            '"><a href="#" class="make_backup_button" data-for="' +
            value +
            '">сделать backup html-кода</a> | <a href="#" class="show_all_backups_button" data-for="' +
            value +
            '" data-backups-count="' +
            backupsCount +
            '">показать все backup&#39;ы (' +
            backupsCount +
            ')</a> <span class="backup_ajax_status"></span></div><div class="jquery_backups_data">' +
            getBackupsData(result, value, 6, "") +
            "</div>";

          if ($("#" + value).is("textarea")) {
            // модификация для texteare с выводом номеров строк

            if ($("#" + value).hasClass("lined"))
              $("#" + value)
                .closest("div.linedwrap")
                .after(content);
    ...
    // метод выводит все backup'ы для указанного поля в админке в скрытом поле
      ...
      if (typeof ajax_data !== "undefined" && ajax_data.length > 0) {
        ...
        $.each(ajax_data, function () {
          if (this.field_name == field_name) {
            if (backupsCountIterator > max_items_count - 1)
              hideBackupClass = " hidden extended";
            ...
            if (backup_id_selected && backup_id_selected == this.id) {
              backupIdSelected =
                '<span class="backup_just_created">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src="/control/public/js/jquery.backup/images/arrow_left_green.png" width="13" height="13" /> &nbsp; <b>backup создан</b></span>';
            ...
            result +=
              ...
              " " +
              this.date_add_time +
              '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a class="show_backup" backupid="showBackup45" href="#">html-код</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a class="remove_backup" data-backup-id="' +
              this.id +
              '" href="#">удалить</a>' +
              backupIdSelected +
              '<br><div id="html_code45" class="backup_code" style="display:none;margin:10px 0"><b>HTML-КОД:</b><br><textarea style="width:95%;height:270px" class="form-control">' +
              this.html_code +
              "</textarea></div></div>";
            ...
            if (backupsCountIterator == max_items_count) {
              result +=
                '<a href="#" class="show_all_backups_extend_button">показать все backup\'ы (' +
                (getBackupsCount(ajax_data, field_name) - max_items_count) +
                ")</a>";
            }
          }
        });
    ...
    $(document.body).delegate(
      ".jquery_backups .make_backup_button",
      "click",
      function () {
        // получаем id поля, backup которго нужно сделать

        var tableName = $(this)
          .closest(".jquery_backups")
        ...
        var backupsDataDiv = $(this)
          .closest(".jquery_backups")
          .next(".jquery_backups_data");

        // устаналиваем ajax preloader

        setAjaxStatus(
          $(this).closest(".jquery_backups").find(".backup_ajax_status")
        );
    ...
    $(document.body).delegate(
      ".jquery_backups .show_all_backups_button",
      "click",
      function () {
    ...
    $(document.body).delegate(
      ".jquery_backups_data .show_all_backups_extend_button",
      "click",
      function () {
        // показываем все backup'ы для текущего поля в админке

        $(this)
          .closest(".jquery_backups_data")
          .find(".extended")
          .toggleClass("hidden");
    ...
    $(document.body).delegate(
      ".jquery_backups_data .show_backup",
      "click",
      function () {
    ...
    $(document.body).delegate(
      ".jquery_backups_data .remove_backup",
      "click",
      function () {
        ...
        if (
          backupID &&
          confirm("Backup будет удален безвозвратно, удалить backup?")
        ) {
          // устаналиваем ajax preloader
          ...
          $.post(
            "/control/public/js/jquery.backup/jquery.backup.ajax.php",
            { action: "removeBackup", backup_id: backupID },
            function (data) {
              if (data == 1) {
                // console.log(data);
                ...
                var backupsCount = $(backupsDataDiv).find(
                  ".backup_item:not(.removed_backups)"
                ).length; // console.log(backupsCount);

                $(backupsDiv)
                  .find(".show_all_backups_button")
                  .text("показать все backup'ы (" + backupsCount + ")");

                $(backupsDiv)
                  .find(".show_all_backups_button")
                  .attr("data-backups-count", backupsCount);
    ...
    function getAllBackupsForSpecificField(
      tableName,
      entryId,
      fieldName,
      backupsDiv,
      ...
      $.post(
        "/control/public/js/jquery.backup/jquery.backup.ajax.php",
        {
          action: "getAllBackupsForSpecificField",
          table_name: tableName,
          entry_id: entryId,
          field_name: fieldName,
        },
        function (data) {
          if (data) {
            ...
            var dataForBackupsDiv = getBackupsData(
              result,
              fieldName,
              6,
              backupIdSelected
            ...
            $(backupsDiv)
              .prev(".jquery_backups")
              .find("a.show_all_backups_button")
              .text("показать все backup'ы (" + backupsCount + ")");
    ...
    function setAjaxStatus(resultElement) {
      ...
      $(document).ajaxStart(function () {
        ...
        $(resultElement).html(
          '<img src="/control/public/js/jquery.backup/images/preloader_1_25x25.gif" width="13" height="13" border="0" />'
        );
      });
