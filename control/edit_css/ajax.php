...

    {
        ...
        try { if ($sth->execute()) {
...

                {
                    ...
                    if (saveContentToFile(PATH_TO_PUBLIC_CSS_OLD.'2.0/',
...

				{
					...
					if (saveContentToFile(PATH_TO_PUBLIC_CSS_OLD,
...

                {
                    ...
                    if (saveContentToFile(PATH_TO_PUBLIC_CSS,
...

			{
				...
				<tr onmouseover="this.style.backgroundColor=\'#cccccc\'" onmouseout="this.style.backgroundColor=\'#ffffff\'">
					...
					<td class="td_edit">
						...
						<a href="/control/edit_css/?action=editItem&itemID='.$_[$i]['id'].'"><img src="/control/images/button_edit.png" style="border:0" /></a>
					...
					<td class="td_remove">
						...
						<a href="/control/edit_css/?action=deleteItem&itemID='.$_[$i]['id'].'" onClick="return confirm(\'Файл css-стилей будет удален безвозвратно. Удалить файл?\')"><img src="/control/images/button_remove.png" style="border:0" /></a>
...

{
	...
	insert into ".DB_PREFIX."css_backups 
...

	{
...

		{
			...
			<div id='".$_[$i]['id']."'>
				...
				&nbsp;&nbsp;&nbsp; <a href='#' class='showBackup' backupID='showBackup".$_[$i]['id']."'>css-код</a>

				&nbsp;&nbsp;&nbsp; <a href='#' class='removeBackup' backupID='removeBackup".$_[$i]['id']."'>удалить</a>

				<br />

				<div id='html_code".$_[$i]['id']."' style='display:none;margin:10px 0'><b>css-код:</b><br /><textarea class='form-control' style='width:95%;height:270px'>".htmlspecialchars($_[$i]['html_code'], ENT_QUOTES)."</textarea></div>
...

	{
...

		{
			...
			<div id='".$_[$i]['id']."'>
				...
				<div id='html_code".$_[$i]['id']."' style='display:none;margin:10px 0'><b>css-код:</b><br /><input class='backup-for-phones' value='".htmlspecialchars($_[$i]['html_code'], ENT_QUOTES)."' /></div>
...

{
	...
	$fullPathToTemplate = PATH_TO_PUBLIC_CSS.basename($fileName); # echo 'fullPathToTemplate: '.$fullPathToTemplate;
...

	{
...

		{
            ...
            <br />Если поле "css-код" оставить пустым, содержимое файла останется "как есть".

            <br />Если поле "css-код" заполнить, css-код будет записан в файл.
...

{
...

	{
...

		{
			...
			echo "<b style='color:red' class='error'>Ошибка</b>: Директория с файлами css-стилей не доступна для записи. Необходимо выставить права: 747.";
