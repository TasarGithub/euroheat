$(document).ready(function() { // jquery ready
    ...
    function checkExistenceByName(event) {
        ...
        if (name.length && name != old_value) {
            ...
            $.post('/control/shows_circus/ajax.php', { 'action': 'check_item_for_existence_by_name', 'name': name }, function(data) {
                if (data) {
                    try {
                        var result = JSON.parse(data); // console.log('%o', result);
                        if (result['result'] == 'exists') {
                            $('#show_form_name_alert_div').html('¬ базе уже существует шоу с названием "' + name + '": <a href="/control/shows_circus/?action=editItem&itemID=' + result['id'] + '" target="_blank">смотреть</a>.<br />ѕжл, укажите другое название дл€ шоу.').removeClass('hidden');
    ...
    $('#search').bind('keyup click', function() {
        ...
        if (q.length > 0 && (!old_data || old_data != q)) {
            ...
            $('#resultSet').html('');
            $.post('/control/shows_circus/ajax.php', { 'action': 'search', 'q': q }, function(data) {
                $('#resultSet').html(data);
