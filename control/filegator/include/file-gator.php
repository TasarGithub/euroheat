...
class gator {
	...
	public function getDirectoryList()
	{
        ...
        # защита
        if (stristr($directory, '/js')
            || stristr($directory, '/css')
            || stristr($directory, '/web-services')
            || stristr($directory, '/vtour')
            ) exit;
		...
		while (false !== ($file = readdir($handler))) {
			...
			if ($file != "."
                && $file != ".."
                && !in_array($file, gatorconf::get('restricted_files'))
                && !preg_match('/[.]css$/i', $file)
                && !preg_match('/[.]js$/i', $file)
                && !preg_match('/[.]php$/i', $file)
                && stristr($file, 'web-services') == false # директори€
                ) {
				...
				if (filetype($directory.DS.$file) == 'dir'){
					...
					if (gatorconf::get('direct_links_baseurl')) {
                        # фикс баги со ссылкой на файлы в корне домашней директории от 11 июл€ 2016
                        if (empty($_GET['cd']) && stristr($directory, gatorconf::get('direct_links_basedir')) === false) {
                            $link = gatorconf::get('direct_links_baseurl').str_replace(gatorconf::get('direct_links_basedir'), '', $directory.'/').$file;
                        }
                        else {
	...
	public function getDirectoryTree($path, $skip_files = false, $link_prefix = '') {
		...
		foreach($objects as $object){
			...
			if ($name == '.'
                || $name == '..'
                || in_array($name, gatorconf::get('restricted_files'))
                # || stristr($name, 'js') # директори€
                || stristr($name, '.js')
                || stristr($name, '.php')
                # || stristr($name, 'css') # директори€
                || stristr($name, '.css')
                || stristr($name, 'web-services') # директори€
                || stristr($name, 'vtour') # директори€
                ) continue;
